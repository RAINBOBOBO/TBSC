...
# Load and merge all archetypes from a parsed result (output of RawParser.parse_file).
func load_archetypes(parsed: Dictionary) -> void:
	for id in parsed:
		assert(not _archetypes.has(id),
			"ArchetypeRegistry: duplicate archetype id '%s' — each archetype id must be unique across all raw files" % id)
		_archetypes[id] = parsed[id]


func get_archetype(id: String) -> Dictionary:
	assert(_archetypes.has(id),
		"ArchetypeRegistry: archetype '%s' not found — check your raw files and make sure it's loaded" % id)
